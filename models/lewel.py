...
from backbone.my_clip import CLIP
...
class EncoderObj(nn.Module):
    def __init__(self, hid_dim, out_dim, norm_layer=None, num_mlp=2,
    ...
    def forward(self, im, caption):
        out = self.backbone(im, caption)
        out = self.neck(out)
        return out


class LEWELB_EMAN(nn.Module):
    def __init__(self, dim=256, m=0.996, hid_dim=4096, norm_layer=None, num_neck_mlp=2,
        ...
        self.online_net = EncoderObj( hid_dim, dim, norm_layer, num_neck_mlp,
                                     scale=scale, l2_norm=l2_norm, num_heads=num_heads)
        self.target_net = EncoderObj(hid_dim, dim, norm_layer, num_neck_mlp,
                                     scale=scale, l2_norm=l2_norm, num_heads=num_heads)
    ...
    def forward(self, im_v1, im_v2=None, caption = None, **kwargs):
        """
        Input:
        ...
        proj_online_v1 = self.online_net(im_v1 , caption)
        proj_online_v2 = self.online_net(im_v2, caption)

        # compute target_net features
        with torch.no_grad():  # no gradient to keys
            proj_target_v1 = [x.clone().detach() for x in self.target_net(im_v1,caption)]
            proj_target_v2 = [x.clone().detach() for x in self.target_net(im_v2,caption)]
