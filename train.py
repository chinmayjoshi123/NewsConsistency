...
from data.base_dataset import get_downstream_split
...
def main():
    ...
    train_transform = data_transforms.Compose([
        data_transforms.RandomResizedCrop(224),
        data_transforms.RandomHorizontalFlip(),
        lambda x: x.convert('RGB'),
        data_transforms.ToTensor(),
        normalize,
    ])
    val_transform = data_transforms.Compose([
        data_transforms.Resize(256),
        data_transforms.CenterCrop(224),
        lambda x: x.convert('RGB'),
        data_transforms.ToTensor(),
        normalize,
    ])
    
    train_dataset = get_downstream_split(confs['dataset'], confs['root'], 
                                                is_train=True, split=confs['split'], transform=train_transform)
